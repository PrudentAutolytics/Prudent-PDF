...
(function(){
  ...
  function expireIdle(){clearSensitiveClientState();try{Object.keys(localStorage).filter(k=>k.startsWith('pr_review_')).forEach(k=>localStorage.removeItem(k))}catch{}window.Session?.clear?.();location.replace('/login?reason=expired')}
